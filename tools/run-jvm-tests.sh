...
echo "== generate the build stamp"
# `BuildInfo` is generated by each real build (see tools/version.sh). The tests
# compile neither build's output, so they get a dev stamp of their own -- the
# class has to exist for the main sources to compile, and a test asserting the
# shipped version would be asserting the tag rather than the code.
cat > "$OUT/gen-buildinfo.kt" <<'EOF'
package io.github.longislandicetea.dshnative

internal object BuildInfo {
    const val VERSION_NAME = "0.0.0-test"
...
echo "$OUT/gen-buildinfo.kt" >> "$OUT/main.list"
